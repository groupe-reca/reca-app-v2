...
- **Factures (Invoices) & Paiements (Payments) — done (first slice, full CRUD; live write test skipped by project owner's choice)**:
  - [x] Real domain models, repository interfaces + Supabase implementations (`SupabaseInvoiceRepository`, `SupabasePaymentRepository`), mappers — `invoices`/`payments` tables typed with full real columns from `reca-app/supabase/migrations/20260709143649_invoices.sql` and `20260709143652_payments.sql`
  - [x] `src/domain/invoiceStatus.ts` — real `InvoiceStatus` (`brouillon`/`envoyee`/`payee`/`partiellement_payee`/`en_retard`/`annulee`) + `paymentMethods` constant, cross-checked against `reca-app`'s real `InvoiceStatusBadge.tsx` and `PAYMENT_METHODS`
  - [x] Money: `invoices.sous_total`/`tps`/`tvq`/`total`/`solde` and `payments.montant` are decimal-dollar columns, same conversion pattern as Contracts/Clients/Quotes. No line-items table in the real schema — invoices are flat amount fields, matching Quotes.
  - [x] **`RecordPayment` deliberately matches `reca-app`'s real non-transactional behavior, not the docs' aspirational transactional RPC**: `docs/16-Development-Standards.md` names `RecordPayment` as an example of a mandated transactional multi-step mutation, but `reca-app`'s real `payments.service.ts` implements it as 3 plain sequential client calls (insert payment → sum non-deleted payments for the invoice → update `invoices.solde`/`statut`), no Postgres RPC or DB transaction. `SupabasePaymentRepository.create()` mirrors this exactly (`recalcInvoiceBalance()` helper), same reasoning as Quotes' non-transactional "link to client" — matching the reference system's actual behavior takes priority over the docs' aspirational example until a real transactional rewrite is separately scoped and decided.
  - [x] No auto-invoice-generation from a contract's payment schedule exists in `reca-app` (checked — no billing-frequency field on `contracts`), so none was built here either; invoices are created manually, optionally linked to a contract.
  - [x] New `InvoiceListPage` (`/invoices`), `InvoiceDetailPage` (`/invoices/:invoiceId`, amounts + payments list + embedded record-payment form + status buttons), `InvoiceCreatePage` (`/invoices/new`, client required + contract optional selects); `PaymentListPage` (`/payments`, global read-only list linking back to invoices) — no standalone payment-create page since payments are only ever recorded from an invoice's detail page, matching the real app's UX
  - [x] `RecordPaymentForm` (`src/features/payments/components/RecordPaymentForm.tsx`) is a reusable component embedded in `InvoiceDetailPage`, not a route — cross-feature invalidation (`useCreatePayment` invalidates both `paymentKeys` and `invoiceKeys`) since Payments is downstream of Invoices in the business chain
  - [x] **Verified live signed in as the real `administrateur` account**: `/invoices` correctly showed 50 real invoices with real client names, dates, and correct money math (e.g. 500,00 $ + 74,88 $ taxes = 574,88 $); opened FAC-000081 showing its real linked contract (CTR-000056), payments panel, and record-payment form all rendering correctly; `/payments` correctly showed 0 (no invoice has any payments yet — all real invoices are still `BROUILLON`, genuine state, consistent with `solde = total` on all of them).
  - [ ] **Live write test (creating a test invoice + recording a payment to verify the recalc logic) was explicitly declined by the project owner this pass** — the create/recalc code is typecheck/lint/test-verified and follows the read-verified pattern exactly, but the balance-recalculation logic itself has not been exercised against real data. Flagged here so a future session doesn't assume it's been live-verified.
  - [x] Both modules code-split via `src/routes/lazyPages.tsx`, consistent with T-005
- **Remaining module**: Paramètres — not yet scoped, per `docs/00-Vision.md` §33 and §14. Real contract _creation_ (replacing the mock wizard) is tracked here too, blocked on Zones/Invoices scope (now less blocked since a real Invoices module exists, but the wizard itself is still untouched — see Contrats module notes above).
