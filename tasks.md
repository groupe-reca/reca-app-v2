...
- **Remaining 4 modules**: not yet scoped. Per `docs/00-Vision.md` §33 and §14: Employés, Équipements, Factures, Paiements, Paramètres — each derived from the Master UI patterns built in T-002, following the same real-schema cross-check process used for Missions/Clients/Routes/Contrats/Leads/Soumissions (read `reca-app`'s migrations, don't guess column names). Real contract _creation_ (replacing the mock wizard) is tracked here too, blocked on Zones/Invoices scope.
